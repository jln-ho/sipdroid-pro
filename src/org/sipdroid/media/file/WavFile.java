...
public class WavFile extends AudioFile {
	...
	public static WavFile openWavFile(InputStream file) throws IOException,
			Exception {
		...
		/*
		 * if (file.length() != chunkSize+8) { throw new
		 * Exception("Header chunk size (" + chunkSize +
		...
		while (true) {
			...
			if (chunkID == FMT_CHUNK_ID) {
				...
				if (compressionCode != 1)
					throw new Exception("Compression Code " + compressionCode
							+ " not supported");
	...
	// Short
	// -------
	public int readFramesMono(short[] sampleBuffer, int numFramesToRead)
			throws IOException, Exception {
		if(numChannels == 2) return readFramesMono(sampleBuffer, 0, numFramesToRead);
		return readFrames(sampleBuffer, 0, numFramesToRead);
		
	}
	
	public int readFramesMono(short[] sampleBuffer, int offset, int numFramesToRead)
			throws IOException, Exception {
		...
		for (int f = 0; f < numFramesToRead; f++) {
			...
			sampleBuffer[offset] = (short) (readSample() + readSample());
			offset++;
			frameCounter++;
		}
	...
	public int readFrames(short[] sampleBuffer, int offset, int numFramesToRead)
			throws IOException, Exception {
		if (ioState != IOState.READING)
			throw new IOException("Cannot read from WavFile instance");
		for (int f = 0; f < numFramesToRead; f++) {
	...
	public int writeFrames(int[][] sampleBuffer, int offset,
			int numFramesToWrite) throws IOException, Exception {
		if (ioState != IOState.WRITING)
			throw new IOException("Cannot write to WavFile instance");
		for (int f = 0; f < numFramesToWrite; f++) {
			if (frameCounter == numFrames)
	...
	public int readFrames(double[][] sampleBuffer, int offset,
			int numFramesToRead) throws IOException, Exception {
		if (ioState != IOState.READING)
			throw new IOException("Cannot read from WavFile instance");
		for (int f = 0; f < numFramesToRead; f++) {
			if (frameCounter == numFrames)
	...
	public String displayStr(){
		return "File: "+ file + "\nChannels: " + numChannels + "\nFrames: " + numFrames + "\nIO State " + ioState +
				"\nSample Rate: " + sampleRate + "\nValid Bits: " + validBits + "\nBytes per sample: " + bytesPerSample;
	}

	@Override
	public boolean isSupported() {
		return validBits == 16 && numChannels <= 2;
	}

	@Override
	protected boolean getFrame(short[] frameBuf) {
		try {
			return readFramesMono(frameBuf, frameBuf.length) > 0;
		} catch (Exception e) {
			e.printStackTrace();
		}
		return false;
