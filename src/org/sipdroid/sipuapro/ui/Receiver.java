...
import org.sipdroid.sipuapro.phone.Call;
import org.sipdroid.sipuapro.SipdroidEngine;
import org.sipdroid.sipuapro.UserAgent;
import org.sipdroid.sipuapro.phone.Connection;
	...
	public class Receiver extends BroadcastReceiver {
			...
			if (call_state != state) {
				...
				case UserAgent.UA_STATE_INCOMING_CALL:
					...
					if ((pstn_state == null || pstn_state.equals("IDLE")) &&
							PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(org.sipdroid.sipuapro.ui.Settings.PREF_AUTO_ON, org.sipdroid.sipuapro.ui.Settings.DEFAULT_AUTO_ON) &&
						...
						if (am.getStreamVolume(AudioManager.STREAM_RING) > 0) {				 
							String sUriSipRingtone = PreferenceManager.getDefaultSharedPreferences(mContext).getString(org.sipdroid.sipuapro.ui.Settings.PREF_SIPRINGTONE,
		...
		public static void onText(int type,String text,int mInCallResId,long base) {
			...
			if (type >= REGISTER_NOTIFICATION && mInCallResId == R.drawable.sym_presence_available &&
					!PreferenceManager.getDefaultSharedPreferences(Receiver.mContext).getBoolean(org.sipdroid.sipuapro.ui.Settings.PREF_REGISTRATION, org.sipdroid.sipuapro.ui.Settings.DEFAULT_REGISTRATION))
				text = null;
	        ...
	        if (text != null) {
				...
				if (type == MISSED_CALL_NOTIFICATION) {
			        	...
			        	if (PreferenceManager.getDefaultSharedPreferences(Receiver.mContext).getBoolean(org.sipdroid.sipuapro.ui.Settings.PREF_NOTIFY, org.sipdroid.sipuapro.ui.Settings.DEFAULT_NOTIFY)) {
					...
					if (base != 0) {
						...
						if (PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(org.sipdroid.sipuapro.ui.Settings.PREF_POS, org.sipdroid.sipuapro.ui.Settings.DEFAULT_POS))
		...
		static void updateAutoAnswer() {
			if (PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(org.sipdroid.sipuapro.ui.Settings.PREF_AUTO_ONDEMAND, org.sipdroid.sipuapro.ui.Settings.DEFAULT_AUTO_ONDEMAND) &&
				Sipdroid.on(mContext)) {
				if (PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(org.sipdroid.sipuapro.ui.Settings.PREF_AUTO_DEMAND, org.sipdroid.sipuapro.ui.Settings.DEFAULT_AUTO_DEMAND))
					updateAutoAnswer(1);
				else
					updateAutoAnswer(0);
			} else
		...
		public static void pos(boolean enable) {
			...
			if (enable) {
				if (call_state == UserAgent.UA_STATE_IDLE && Sipdroid.on(mContext) &&
						PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(org.sipdroid.sipuapro.ui.Settings.PREF_POS, org.sipdroid.sipuapro.ui.Settings.DEFAULT_POS) &&
						PreferenceManager.getDefaultSharedPreferences(mContext).getString(org.sipdroid.sipuapro.ui.Settings.PREF_POSURL, org.sipdroid.sipuapro.ui.Settings.DEFAULT_POSURL).length() > 0) {
		...
		static void enable_wifi(boolean enable) {
			if (!PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(org.sipdroid.sipuapro.ui.Settings.PREF_OWNWIFI, org.sipdroid.sipuapro.ui.Settings.DEFAULT_OWNWIFI))
				return;
			if (enable && !PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(org.sipdroid.sipuapro.ui.Settings.PREF_WIFI_DISABLED, org.sipdroid.sipuapro.ui.Settings.DEFAULT_WIFI_DISABLED))
        		return;
    		...
    		edit.putBoolean(org.sipdroid.sipuapro.ui.Settings.PREF_WIFI_DISABLED,!enable);
    		edit.commit();
    		if (enable) {
		...
		public static void url(final String opt) {
	        (new Thread() {
				public void run() {
					try {
				        URL url = new URL(PreferenceManager.getDefaultSharedPreferences(mContext).getString(org.sipdroid.sipuapro.ui.Settings.PREF_POSURL, org.sipdroid.sipuapro.ui.Settings.DEFAULT_POSURL)+
				        		"?"+opt);
				        BufferedReader in;
		...
		static boolean on_vpn() {
			return PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(org.sipdroid.sipuapro.ui.Settings.PREF_ON_VPN, org.sipdroid.sipuapro.ui.Settings.DEFAULT_ON_VPN);
		}
		
		static void on_vpn(boolean enable) {
    		Editor edit = PreferenceManager.getDefaultSharedPreferences(Receiver.mContext).edit();
    		
    		edit.putBoolean(org.sipdroid.sipuapro.ui.Settings.PREF_ON_VPN,enable);
    		edit.commit();
		}
		...
		public static boolean isFast(int i) {
        	...
        	if (PreferenceManager.getDefaultSharedPreferences(mContext).getString(org.sipdroid.sipuapro.ui.Settings.PREF_USERNAME+(i!=0?i:""),"").equals("") ||
        			PreferenceManager.getDefaultSharedPreferences(mContext).getString(org.sipdroid.sipuapro.ui.Settings.PREF_SERVER+(i!=0?i:""),"").equals(""))
        		return false;
        	if (wi != null) {
	        	...
	        	if (wi.getIpAddress() != 0 && (WifiInfo.getDetailedStateOf(wi.getSupplicantState()) == DetailedState.OBTAINING_IPADDR
	        		...
	        		on_wlan = true;
	        		if (!on_vpn())
	        			return PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(org.sipdroid.sipuapro.ui.Settings.PREF_WLAN+(i!=0?i:""), org.sipdroid.sipuapro.ui.Settings.DEFAULT_WLAN);
	        		else
	        			return PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(org.sipdroid.sipuapro.ui.Settings.PREF_VPN+(i!=0?i:""), org.sipdroid.sipuapro.ui.Settings.DEFAULT_VPN);  
	        	}
        	}
        	on_wlan = false;
			return isFastGSM(i);
		...
		static boolean isFastGSM(int i) {
        	...
        	if (tm.getNetworkType() >= TelephonyManager.NETWORK_TYPE_UMTS)
        		return PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(org.sipdroid.sipuapro.ui.Settings.PREF_3G+(i!=0?i:""), org.sipdroid.sipuapro.ui.Settings.DEFAULT_3G);
        	if (tm.getNetworkType() == TelephonyManager.NETWORK_TYPE_EDGE)
       			return PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(org.sipdroid.sipuapro.ui.Settings.PREF_EDGE+(i!=0?i:""), org.sipdroid.sipuapro.ui.Settings.DEFAULT_EDGE);
        	return false;
		}
		...
		public void onReceive(Context context, Intent intent) {
	        ...
	        if (intentAction.equals(WifiManager.SCAN_RESULTS_AVAILABLE_ACTION)) {
	        	if (SystemClock.uptimeMillis() > lastscan + 45000 &&
	        			PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(org.sipdroid.sipuapro.ui.Settings.PREF_SELECTWIFI, org.sipdroid.sipuapro.ui.Settings.DEFAULT_SELECTWIFI)) {
		        	WifiManager wm = (WifiManager) mContext.getSystemService(Context.WIFI_SERVICE);
		        	WifiInfo wi = wm.getConnectionInfo();
